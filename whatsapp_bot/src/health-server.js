 ...
 */
function createHealthServer({ service, registry, port }) {
    const server = http.createServer(async (req, res) => {
        ...
        try {
            ...
            if (route === 'GET /') return sendHtml(res, await renderPanel(service, registry));
...
async function renderPanel(service, registry) {
...
<style>
  ...
  .card { border:1px solid rgba(127,127,127,.35); border-radius:12px; padding:16px; margin:16px 0; max-width:640px; }
